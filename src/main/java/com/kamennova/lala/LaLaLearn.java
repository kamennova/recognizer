...
public class LaLaLearn {
    ...
    public void processInput(String inputPath) throws Exception {
        ...
        ChordSeqFull normalized = normalize(melodyTrack);
        ...
        List<NoteSeqFull> sequences3 = getSequences(normalized, 3);
    ...
    private void storeSequences(List<NoteSeqFull> seq) {
        seq.forEach(notes -> store3.put(notes.get().stream().map(n -> Math.toIntExact(n.interval)).collect(Collectors.toList()),
